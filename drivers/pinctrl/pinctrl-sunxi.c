...
{
	struct sunxi_pinctrl *pctl = pinctrl_dev_get_drvdata(pctldev);
	return sunxi_pinconf_set(pctldev, pctl->groups[group].pin, configs, num_configs);
}			          
...
static int sunxi_pinctrl_parse_pin_cfg(struct platform_device *pdev)
{
	...
	for (mainkey_idx = 0; mainkey_idx < mainkey_count; mainkey_idx++) {
		...
		if(script_is_main_key_exist(mainkey_suspend)){
			...
			/* build suspend state */
			state_name = PINCTRL_STATE_SLEEP;
			ret = sunxi_pinctrl_creat_mappings(pdev, pin_count, pin_list, device_name, state_name);
		}else{
			state_name = PINCTRL_STATE_SLEEP;
...
static int pin_config_set(const char *dev_name, const char * pin_name,
	...
	if (!pctldev)
		return -EINVAL;
	pin = pin_get_from_name(pctldev, pin_name);
	if (pin < 0)
		return pin;
	sunxi_pinconf_set(pctldev, pin, configs, num_configs);
	return 0;
}

static int pin_config_get(const char *dev_name, const char *pin_name,
		unsigned long *config) {
	int pin;
	struct pinctrl_dev *pctldev;
	pctldev = get_pinctrl_dev_from_devname(dev_name);
	...
	if (pin < 0)
		return pin;
	sunxi_pinconf_get(pctldev, pin, config);
	return 0;
}
	...
	const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	configs[0] = SUNXI_PINCFG_PACK(SUNXI_PINCFG_TYPE_FUNC,function);
	pin_config_set(SUNXI_PINCTRL,sunxi_dbg_pinname,configs,1);
	return count;
}
	...
	const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	configs[0] = SUNXI_PINCFG_PACK(SUNXI_PINCFG_TYPE_DAT,data);
	pin_config_set(SUNXI_PINCTRL,sunxi_dbg_pinname,configs,1);
	...
	const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	configs[0] = SUNXI_PINCFG_PACK(SUNXI_PINCFG_TYPE_DRV,dlevel);
	pin_config_set(SUNXI_PINCTRL,sunxi_dbg_pinname,configs,1);
	...
	const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	configs[0] = SUNXI_PINCFG_PACK(SUNXI_PINCFG_TYPE_PUD,pull);
	pin_config_set(SUNXI_PINCTRL,sunxi_dbg_pinname,configs,1);
